<template>
  <div class="container">
    ...
    <button
      class="cycle-control outer-shadow"
      @click="drawStroke(), rotateIndicator(), scaleIndicator()"
    >
      TEST
    </button>
  ...
  methods: {
    ...
    scaleIndicator() {
      gsap.fromTo(
        "#circle-inner_fill",
        { scale: 1 },
        { scale: 1.75, transformOrigin: "50% 50%" }
      );
    },
    ...
    drawStroke() {
      gsap.fromTo(
        ".branch",
        { drawSVG: "0% 0%" },
        { duration: 1, drawSVG: "0% 100%", stagger: 0.5, ease: "power1.out" }
      );
