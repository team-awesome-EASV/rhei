<template>
  <div>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 581.9 641.71">
      <g id="Layer_2" data-name="Layer 2">
        <g id="Layer_3" data-name="Layer 3">
          <g id="background" opacity="0.12">
            <g id="Background-2" data-name="Background">
              <circle
                cx="283.44"
                cy="358.26"
                r="283.44"
                :fill="createShade('46', '50')"
              />
            </g>
          </g>
          <g id="chair">
            <polygon
              points="276.89 571.39 114.89 475.39 115.89 328.39 274.89 348.39 276.89 571.39"
              fill="var(--secondary-background-color)"
            />
            <polygon
              points="274.89 348.39 276.89 571.39 437.89 476.39 436.89 326.39 274.89 348.39"
              fill="#d3d3d3"
            />
            <polyline
              points="115.89 328.39 276.89 303.39 436.89 326.39 274.89 377.39"
              fill="#757575"
            />
          </g>
          <g id="chart">
            <g id="chart-circle" opacity="0.5">
              <g id="chart-outside">
                <path
                  id="chart3"
                  d="M336.63,115.87a113.32,113.32,0,0,1,98.23-95"
                  fill="none"
                  :stroke="createShade('45', '25')"
                  stroke-linecap="round"
                  stroke-miterlimit="10"
                  stroke-width="40"
                />
                <path
                  id="chart2"
                  d="M374.33,47.73A113.3,113.3,0,1,1,363,207.57"
                  fill="none"
                  :stroke="createShade('46', '50')"
                  stroke-linecap="round"
                  stroke-miterlimit="10"
                  stroke-width="40"
                />
                <path
                  id="chart1"
                  d="M374.33,47.73a113.31,113.31,0,0,1,186.86,98.2"
                  fill="none"
                  :stroke="createShade('45', '90')"
                  stroke-linecap="round"
                  stroke-miterlimit="10"
                  stroke-width="40"
                />
              </g>
              <circle
                id="chart-center"
                cx="448.58"
                cy="133.31"
                r="61.63"
                :fill="createShade('90', '50')"
              />
            </g>
          </g>
          <g id="character">
            <g id="legs">
              <path
                id="shoe1"
                d="M383.45,477.36c-.26,1.45-2.22,9.94-1.24,11.6,1.76,3,20.07,5.07,21.07,4.38s1.83-2.38,1.17-3-7.6-3.4-8.45-4.26-1.56-4.68-.69-6.52"
                fill="#1f161e"
              />

              <path
                id="leg-back"
                d="M249.91,299.65c0,54,17.18,65.34,34.27,65.34,43.29,0,99.61-34.72,102.79-36.06-1,5.68-7.31,149.66-7.31,150.49,1.5,1.17,13,2.18,16.87,1.17,1-3.5,31.41-174.75,31.41-179.26S425.32,290,415.41,289c-4.19-.42-62.48-2.84-62.48-2.84"
                :fill="createShade('20', '30')"
              />

              <g id="leg-front">
                <path
                  id="shoe2"
                  d="M454.19,422.6c.59,1.35,3.73,9.47,5.47,10.3,3.15,1.49,19.46-7.06,19.9-8.19s.19-3-.73-3.16-8.2,1.44-9.38,1.21-3.91-3-4.23-5"
                  fill="#1f161e"
                />
                <path
                  d="M396.84,261.36C410,280.36,468.6,413,468.08,416.09s-14,9.67-16.85,7.9c-12.09-7.45-86.32-132.63-89.21-134.52s-7.22-1.16-9.8-.64C345.51,277.68,360.53,264.44,396.84,261.36Z"
                  :fill="createShade('100', '30')"
                />
              </g>
            </g>
            <path
              id="torso"
              d="M338.06,154.92c19.58,0,25.21,18.54,26.83,25.54,5.18,22.35-9.24,107.28-10.91,109.62-14.17,19.81-95.17,21.57-103.07,13S284.56,154.92,338.06,154.92Z"
              :fill="createShade('90', '50')"
            />
            <g id="head">
              <path
                id="neck"
                d="M335.13,158.83c-.53,1.7,13.29,6,13.82,4.31l5.36-17.2-13.81-4.31Z"
                fill="#f8cdba"
              />
              <path
                id="neck-shadow"
                d="M348.83,150c-1.35-.42-11.16.71-11.82,2.83l3.49-11.21C342.65,142.3,348.83,150,348.83,150Z"
                fill="#1f161e"
              />
              <circle
                id="head-2"
                data-name="head"
                cx="356.54"
                cy="132.48"
                r="20.29"
                fill="#f8cdba"
              />
              <ellipse
                id="cheek2"
                cx="373.38"
                cy="142.1"
                rx="1.85"
                ry="3.56"
                transform="translate(126.5 456.2) rotate(-72.67)"
                fill="#fa9e9f"
              />
              <ellipse
                id="cheek1"
                cx="352.04"
                cy="135.45"
                rx="1.85"
                ry="3.56"
                transform="translate(117.88 431.17) rotate(-72.67)"
                fill="#fa9e9f"
              />
              <path
                id="nose"
                d="M366.25,135.87c.35,1.1,2.43,5.92-1.48,6.39"
                fill="none"
                stroke="#1f161e"
                stroke-linecap="round"
                stroke-miterlimit="10"
                stroke-width="1.2"
              />
              <path
                id="mouth"
                d="M356.53,144.56c-1-.35-2.2-2.07-2.67-3.48"
                fill="none"
                stroke="#1f161e"
                stroke-linecap="round"
                stroke-miterlimit="10"
                stroke-width="1.2"
              />
              <path
                id="hair"
                d="M362.68,105.86c-5.35-.67-20.88-5.29-24.41-8.5-1.67.88-8.16,6.71-13.45,14.6l7.51,7.55-9.49-4.38a38.74,38.74,0,0,0-5.07,13.35c2.45,6.61,10.14,9.33,11.59,9.51,9.35,1.18,12.94-13.35,17.65-15.15,8-3.07,17.42,4.9,29.27,4.4C378.91,121.87,375.49,107.47,362.68,105.86Z"
                fill="#1f161e"
              />
              <circle
                id="ear"
                cx="337.94"
                cy="129.12"
                r="4.69"
                fill="#f8cdba"
              />
            </g>
            <g id="arm">
              <rect
                id="phone"
                x="416.57"
                y="234.2"
                width="32.18"
                height="14.77"
                transform="translate(61.3 569.7) rotate(-70.64)"
                fill="#1f161e"
              />
              <circle
                id="hand"
                cx="434.11"
                cy="257.04"
                r="10.87"
                fill="#f8cdba"
              />
              <path
                id="arm-fill"
                d="M317.77,202.48c-1,6.26-4.24,65.37,3.52,71.12,16.62,12.31,93.37-2.75,105-7.06,1.26-4.12,0-17.21,0-17.21-4.48-.18-61-.72-64.37-5s-3.77-39.27-3.23-43.22"
                :fill="createShade('90', '50')"
              />
              <path
                id="arm-contour-2"
                d="M361.91,244.36c-3.41-4.31-3.77-39.27-3.23-43.22"
                fill="none"
                stroke="#1f161e"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                id="arm-contour-1"
                d="M356.6,278c-15.66.87-29.35,0-35.31-4.44-3.4-2.51-4.69-15.24-5-29.29"
                fill="none"
                stroke="#1f161e"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
            </g>
          </g>
          <g id="phone-2" data-name="phone">
            <g id="phone-bcg">
              <rect
                id="phone-bcg-frame"
                x="40.46"
                y="25.6"
                width="161.19"
                height="326.51"
                rx="17.77"
                :fill="createShade('90', '20')"
              />
              <rect
                x="50.35"
                y="35.08"
                width="142.64"
                height="307.96"
                rx="12.86"
                fill="var(--secondary-background-color)"
              />
              <rect
                x="81.48"
                y="25.39"
                width="79.57"
                height="19.38"
                rx="7.78"
                :fill="createShade('90', '20')"
              />
              <line
                x1="87.46"
                y1="337.48"
                x2="155.89"
                y2="337.48"
                :fill="createShade('90', '20')"
                :stroke="createShade('95', '40')"
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="6.79"
              />
            </g>
            <circle
              id="circle"
              cx="121.67"
              cy="104.34"
              r="41.43"
              :fill="createShade('90', '80')"
            />
            <g id="ui-group1">
              <polyline
                class="ui-line"
                points="67.05 175.46 67.05 158.14 213.61 158.14 213.61 175.46 67.05 175.46"
                :fill="createShade('90', '39')"
              />
              <polyline
                class="ui-line"
                points="141.83 205.14 67.05 205.14 67.05 196.48"
                fill="none"
                stroke="#000"
                stroke-miterlimit="10"
                stroke-width="1.31"
              />
              <polyline
                class="ui-line"
                points="67.05 187.82 213.61 187.82 213.61 196.48"
                fill="none"
                stroke="#000"
                stroke-miterlimit="10"
                stroke-width="1.31"
              />
            </g>
            <g id="ui-group2">
              <polyline
                class="ui-line"
                points="141.83 255.84 67.05 255.84 67.05 247.19"
                fill="none"
                stroke="#000"
                stroke-miterlimit="10"
                stroke-width="1.31"
              />
              <polyline
                class="ui-line"
                points="67.05 268.21 213.61 268.21 213.61 276.87"
                fill="none"
                stroke="#000"
                stroke-miterlimit="10"
                stroke-width="1.31"
              />
              <rect
                class="ui-line"
                x="67.05"
                y="217.5"
                width="146.56"
                height="17.31"
                :fill="createShade('90', '39')"
              />
            </g>
          </g>
        </g>
      </g>
    </svg>
  </div>
</template>

<script>
import { createShadeAccentColor } from "../mixins/createShadeAccentColor";
import { gsap } from "gsap";
export default {
  name: "TheCalmDownIllu",
  mixins: [createShadeAccentColor],
  data() {
    return {
      masterTL: gsap.timeline({ paused: true })
    };
  },

  methods: {
    rotateChart() {
      let tl = gsap.timeline();

      tl.to("#chart-outside", {
        rotation: 360,
        duration: 5,
        repeat: -1,
        yoyo: true,
        ease: "back.inOut(1.7)",
        transformOrigin: "50% 50%"
      });

      return tl;
    },

    legSwing() {
      let tl = gsap.timeline();
      tl.to("#leg-front", {
        rotation: -10,
        duration: 2,
        repeat: -1,
        yoyo: true,
        ease: "back.inOut(1.7)",
        transformOrigin: "0% 0%"
      });
      return tl;
    },

    interfaceDance() {
      let tl = gsap.timeline();
      tl.to(".ui-line", {
        scaleX: 0.1,
        duration: 1,
        repeat: -1,
        yoyo: true,
        ease: "back.inOut(1.7)",
        transformOrigin: "0% 0%",
        stagger: {
          from: "random",
          each: 0.3,
          amount: 1
        }
      });
      return tl;
    },

    scaleBackground() {
      let tl = gsap.timeline();
      tl.to("Background-2", {
        scale: 10,
        duration: 2,
        repeat: -1,
        yoyo: true,
        ease: "back.inOut(1.7)",
        transformOrigin: "50% 50%"
      });
      return tl;
    }
  },

  mounted() {
    this.masterTL.add(this.rotateChart());
    this.masterTL.add(this.legSwing(), 0);
    this.masterTL.add(this.interfaceDance(), 0);
    this.masterTL.add(this.scaleBackground(), 0);
    this.masterTL.play();
    // this.animPlay;
    // gsap.to("#chart-outside", {
    //   rotation: 360,
    //   duration: 5,
    //   repeat: -1,
    //   yoyo: true,
    //   ease: "back.inOut(1.7)",
    //   transformOrigin: "50% 50%"
    // });
  }
};
</script>

<style lang="scss" scoped>
</style>