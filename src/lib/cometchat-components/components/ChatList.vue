<template>
  <div>
    ...
    >
      <div
        class="chat-ppl-list-wrap"
        v-for="(chatRow, index) in chatList"
        :key="index"
      >
        <div
          class="chat-ppl-listitem clearfix"
          v-on:click="currentUser(chatRow, index)"
        >
          <Avatar :item="chatRow.conversationWith" />
          <div class="chat-ppl-listitem-dtls">
            <span class="chat-ppl-listitem-name">{{
              chatRow.conversationWith.name
            }}</span>
            <p
              class="chat-ppl-listitem-txt ccl-secondary-color"
              v-if="chatRow.lastMessage"
            >
              {{ chatRow.lastMessage.data.text }}
            </p>
            <p class="chat-ppl-listitem-space" v-else-if="!chatRow.lastMessage">
              &nbsp;
            </p>
          </div>
          <span
            class="chat-ppl-listitem-msg-cnt"
            v-if="chatRow.unreadMessageCount > 0"
            >{{ chatRow.unreadMessageCount }}</span
          >
        </div>
      </div>
    </div>
  ...
  data() {
    return {
      ...
      callChatList(searchKey) {
        if (searchKey) {
          this.chatRequest = new CometChat.ConversationsRequestBuilder()
            .setLimit(30)
  ...
  methods: {
    ...
    handleChatScroll(element) {
      if (
        ...
        element.target.scrollHeight - 20
      ) {
        this.chatFetchNext().then(
          chat => {
            this.chatList = [...this.chatList, ...chat];
          },
          error => {
